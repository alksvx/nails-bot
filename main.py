...
def starter(message):
    ...
    letsgo.row(lets)
    bot.send_message(message.chat.id, "Привет, это наш маникюр-бот.", reply_markup=letsgo)
    ...
    bot.register_next_step_handler(message, formanails)
...
def menuli(message):
    if message.text == 'Еще!':
        ...
        bot.register_next_step_handler(message, appointment)
            ...
            bot.send_message(message.chat.id, "Выбери длину", reply_markup=length)
            bot.register_next_step_handler(message, formanails1)

def formanails1(message):
    global forma
    forma = [message.text]
    form = types.ReplyKeyboardMarkup(resize_keyboard=True)
    square = types.KeyboardButton("Квадратная")
    ...
    bot.send_photo(message.chat.id, pic)
    bot.send_message(message.chat.id, 'Выбери форму', reply_markup=form)
    bot.register_next_step_handler(message, color1)
...
def color1(message):
    ...
    bot.register_next_step_handler(message, design1)
...
def design1(message):
    ...
    bot.register_next_step_handler(message, ready1)
